...
library RiscVDecoder {
    ...
    function insnRd(uint32 insn) public returns(uint32) {
        return (insn >> 7) & 0x1F;
    }

    /// @notice Get the instruction's RS1
    //  @param insn Instruction
    function insnRs1(uint32 insn) public returns(uint32) {
        return (insn >> 15) & 0x1F;
    }

    /// @notice Get the instruction's RS2
    //  @param insn Instruction
    function insnRs2(uint32 insn) public returns(uint32) {
        return (insn >> 20) & 0x1F;
    }

    /// @notice Get the I-type instruction's immediate value
    //  @param insn Instruction
    function insnIImm(uint32 insn) public returns(int32) {
    ...
    //  @param insn Instruction
    function insnIUimm(uint32 insn) public returns(uint32) {
        return insn >> 20;
    }

    /// @notice Get the U-type instruction's immediate value
    //  @param insn Instruction
    function insnUImm(uint32 insn) public returns(int32) {
        return int32(insn & 0xfffff000);
    }

    /// @notice Get the B-type instruction's immediate value
    //  @param insn Instruction
    function insnBImm(uint32 insn) public returns(int32) {
        int32 imm = int32(
            ((insn >> (31 - 12)) & (1 << 12)) |
            ((insn >> (25 - 5)) & 0x7e0) |
            ((insn >> (8 - 1)) & 0x1e) |
    ...
    //  @param insn Instruction
    function insnJImm(uint32 insn) public returns(int32) {
        int32 imm = int32(
            ((insn >> (31 - 20)) & (1 << 20)) |
            ((insn >> (21 - 1)) & 0x7fe) |
            ((insn >> (20 - 11)) & (1 << 11)) |
